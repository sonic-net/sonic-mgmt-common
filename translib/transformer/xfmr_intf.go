...
var DbToYang_intf_ipv4_counters_xfmr SubTreeXfmrDbToYang = func(inParams XfmrParams) error {
	...
	for _, member := range members {
		...
		if err := populatePortCounters(inParams, member, &mcounters); err != nil {
			log.Warningf("Failed to populate IPv4 counters for member %s (interface %s): %v", member, ifName, err)
			continue
		}
...
var DbToYang_intf_ipv6_counters_xfmr SubTreeXfmrDbToYang = func(inParams XfmrParams) error {
	...
	for _, member := range members {
		...
		if err := populatePortCounters(inParams, member, &mcounters); err != nil {
			log.Warningf("Failed to populate IPv6 counters for member %s (interface %s): %v", member, ifName, err)
			continue
		}
