...
func TestSubscribe_intf_get_counters_xfmr(t *testing.T) {
	tests := []struct {
		...
		expectError bool
		errorMsg    string
		validateOut func(t *testing.T, res XfmrSubscOutParams)
	}{
		{
			name:        "Success - Specific Interface Key Subscription Mapping",
			uri:         "/openconfig-interfaces:interfaces/interface[name=Ethernet202]/state/counters",
			setupMock:   func() {},
			expectError: false,
			validateOut: func(t *testing.T, res XfmrSubscOutParams) {
				if res.needCache != true || res.onChange != OnchangeDisable {
					t.Errorf("Subscription parameter flags were incorrectly assigned")
				}

				tblMap, exists := res.dbDataMap[db.ConfigDB]
				if !exists {
					t.Fatalf("ConfigDB key missing from output subscription map framework")
				}

				if len(tblMap) == 0 {
					t.Fatalf("Expected at least one table entry in the configuration subscription map")
				}

				var foundIntf bool
				for tblName, interfaceMap := range tblMap {
					if _, ok := interfaceMap["Ethernet202"]; ok {
						foundIntf = true
						t.Logf("Successfully verified interface entry inside table: %s", tblName)
						break
					}
				}

				if !foundIntf {
					t.Errorf("Expected subscription path for interface key 'Ethernet202' not found across generated tables")
				}
			},
		},
		{
			name:        "Failure - Invalid Interface String Target",
			uri:         "/openconfig-interfaces:interfaces/interface[name=InvalidIntf99]/state/counters",
			setupMock:   func() {},
			expectError: true,
			errorMsg:    "Invalid interface:",
		...
		{
			name:        "Success - Wildcard Interface Pattern Expansion Mapping",
			...
			validateOut: func(t *testing.T, res XfmrSubscOutParams) {
				tblMap, exists := res.dbDataMap[db.ConfigDB]
				if !exists {
					t.Fatalf("ConfigDB subscription data framework initialization skipped")
				}
				if len(tblMap) == 0 {
					t.Errorf("Wildcard loop expansion generated an empty subscription schema map")
				}
				for _, tbl := range tblMap {
					if _, hasWildcard := tbl["*"]; !hasWildcard {
						t.Errorf("Wildcard interface key marker '*' missing from generated sub-table layout")
					}
				}
			},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectError {
				if err == nil {
					t.Fatalf("Expected subscription configuration fault but function returned success")
				}
				if !strings.Contains(err.Error(), tt.errorMsg) {
			...
			if err != nil {
				t.Fatalf("Unexpected execution fault returned during parameters layout calculation: %v", err)
			}

			if tt.validateOut != nil {
				tt.validateOut(t, res)
			}
		})
	}
}
...
func TestSubscribe_intf_get_ether_counters_xfmr(t *testing.T) {
	tests := []struct {
		name        string
		uri         string
		expectError bool
		...
		{
			name:        "Success - Specific Interface Extraction Path",
			uri:         "/openconfig-interfaces:interfaces/interface[name=Ethernet202]/ethernet/state/counters",
			expectError: false,
			validateOut: func(t *testing.T, res XfmrSubscOutParams) {
				if res.needCache != true || res.onChange != OnchangeDisable {
					t.Errorf("Subscription parameter attributes were incorrectly initialized")
				}

				tblMap, exists := res.dbDataMap[db.ConfigDB]
				if !exists {
					t.Fatalf("ConfigDB key missing from generated subscription map")
				}

				interfaceMap, hasPortTbl := tblMap["PORT"]
				if !hasPortTbl {
					t.Fatalf("Expected target database table 'PORT' was not generated")
				}

				if _, ok := interfaceMap["Ethernet202"]; !ok {
					t.Errorf("Subscription key entry for interface 'Ethernet202' was not found")
				}
			},
		},
		{
			name:        "Success - Fallback to Wildcard Expansion when Name is Empty",
			uri:         "/openconfig-interfaces:interfaces/interface/ethernet/state/counters",
			expectError: false,
			validateOut: func(t *testing.T, res XfmrSubscOutParams) {
				tblMap, exists := res.dbDataMap[db.ConfigDB]
				if !exists {
					t.Fatalf("ConfigDB key missing from empty path subscription layout")
				}

				interfaceMap, hasPortTbl := tblMap["PORT"]
				if !hasPortTbl {
					t.Fatalf("Expected target database table 'PORT' was missing from fallback setup")
				}

				if _, ok := interfaceMap["*"]; !ok {
					t.Errorf("Expected wildcard string modifier '*' fallback was not assigned when name segment was missing")
				}
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			res, err := Subscribe_intf_get_ether_counters_xfmr(inParams)

			if tt.expectError && err == nil {
				t.Fatalf("Expected subscription execution failure state but function completed successfully")
			}
			if !tt.expectError && err != nil {
				t.Fatalf("Unexpected execution context error returned: %v", err)
			}
