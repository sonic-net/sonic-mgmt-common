...
func Test_Portchannel_GetLagState(t *testing.T) {
...
func Test_Portchannel_DbToYang_intf_lag_state_xfmr(t *testing.T) {
	...
	setupData := fmt.Sprintf(`{
		"PORTCHANNEL": {"%s": {"min_links": "1", "lag_type": "LACP"}},
...
func Test_Portchannel_YangToDb_lag_type_xfmr(t *testing.T) {
	...
	t.Run("New_PortChannel(Not_in_DB)", func(t *testing.T) {
	...
	t.Run("Existing_PortChannel_Match", func(t *testing.T) {
	...
	t.Run("Conflict_Error(LACP_to_STATIC)", func(t *testing.T) {
		d.SetEntry(&db.TableSpec{Name: "PORTCHANNEL"}, db.Key{Comp: []string{uniquePC}}, db.Value{Field: map[string]string{"lag_type": "LACP"}})
		defer d.DeleteEntry(&db.TableSpec{Name: "PORTCHANNEL"}, db.Key{Comp: []string{uniquePC}})
	...
	t.Run("Conflict_Error(STATIC_to_LACP)", func(t *testing.T) {
...
func Test_Portchannel_fillLagInfoForIntf(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setupJSON: `{
				"PORTCHANNEL": {"PortChannel_UT_Fill1": {"min_links": "2", "lag_type": "STATIC"}},
...
func Test_Portchannel_getLagStateAttr_MemberLogic(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:           "Valid Members",
			attr:           "member",
			memberValue:    "Ethernet0,Ethernet4",
			includeKey:     true,
			expectedResult: []string{"Ethernet0", "Ethernet4"},
		},
		{
			name:           "Empty String Guard (The Fix)",
			attr:           "member",
			memberValue:    "",
			includeKey:     true,
			expectedResult: nil,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.includeKey {
				fields["member@"] = tt.memberValue
			}
			lagInfoMap[ifName] = db.Value{Field: fields}

			err := getLagStateAttr(ptr(tt.attr), &ifName, lagInfoMap, ocVal)

			if err != nil {
				t.Fatalf("getLagStateAttr failed: %v", err)
			}

			if tt.expectedResult == nil {
				if ocVal.Member != nil {
					t.Errorf("%s: Expected Member list to be nil (omitted), but got %v", tt.name, ocVal.Member)
				}
			} else {
				if len(ocVal.Member) != len(tt.expectedResult) {
					t.Errorf("%s: Length mismatch. Expected %d, Got %d", tt.name, len(tt.expectedResult), len(ocVal.Member))
				}
				for i := range tt.expectedResult {
					if ocVal.Member[i] != tt.expectedResult[i] {
						t.Errorf("%s: Member mismatch at index %d. Expected %s, Got %s", tt.name, i, tt.expectedResult[i], ocVal.Member[i])
					}
				}
			}
		})
