...
func (d *DB) getEntry(ts *TableSpec, key Key, forceReadDB bool) (Value, error) {
	...
	if !cacheHit && !txCacheHit {
		if d.readCacheOnlyForTable(ts) && !forceReadDB {
			glog.V(3).Infof("GetEntry(): cache miss but cache write is disabled for %v", ts.Name)
...
func (d *DB) doWrite(ts *TableSpec, op _txOp, k Key, val interface{}) error {
	...
	case txOpHMSet, txOpHDel:
		...
		if _, ok := d.txTsEntryMap[ts.Name][entry]; !ok {
			var v map[string]string
			glog.V(3).Info("doWrite: RedisCmd: ", d.Name(), ": ", "HGETALL ", d.key2redis(ts, key))
...
func (d *DB) performWatch(w []WatchKeys, tss []*TableSpec) error {
	...
	for i := 0; i < len(w); i++ {
		...
		glog.V(3).Info("performWatch: RedisCmd: ", d.Name(), ": ", "KEYS ", redisKey)
	...
	// Issue the WATCH
	glog.V(3).Info("performWatch: Do: ", args)
...
func doDels(txCmds []_txCmd, tsmap map[TableSpec]bool, d *DB) error {
	...
	for i := 0; i < len(d.txCmds); i++ {
		...
		if d.txCmds[i].op == txOpDel {
			...
			glog.V(3).Info("CommitTx: Do: ", args)
...
func (d *DB) commitTx() error {
	...
	// Issue MULTI
	glog.V(3).Info("CommitTx: Do: MULTI")
	if _, e = d.client.Do(context.Background(), "MULTI").Result(); e != nil {
		glog.V(0).Info("CommitTx: Do: MULTI e: ", e.Error())
	...
	if e = doDels(d.txCmds, tsmap, d); e != nil {
		glog.V(0).Info("CommitTx: doDels error", e.Error())
		goto CommitTxDiscard
	}

	if e = doRemainingOps(d.txCmds, tsmap, d); e != nil {
		glog.V(0).Info("CommitTx: doSets error", e.Error())
		goto CommitTxDiscard
	}
	...
	// Issue EXEC
	glog.V(3).Info("CommitTx: Do: EXEC")
	if _, e = d.client.Do(context.Background(), "EXEC").Result(); e != nil {
		glog.V(1).Info("CommitTx: Do: EXEC e: ", e.Error())
...
CommitTxDiscard:
	if e != nil {
		...
		if de != nil {
			glog.V(0).Info("CommitTx: End: Error in discard MULTI queue, e: ", de.Error())
		}
	}
...
func (d *DB) abortTx() error {
	...
	// Issue UNWATCH
	glog.V(3).Info("AbortTx: Do: UNWATCH")
